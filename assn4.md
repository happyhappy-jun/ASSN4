...
오버플로우가 일어나서, 음수로 나타나는 것이였다. 그래서 계산 중의 사용하는 중간 값들의 타입을 ```unsigned long long```으로 변경하였다. 아마 실제로 이 타입의 맥스값까지 사용하지는 않을 테지만여 (실제로 테스트해본 결과 크기가 커도 ``ùnsigned int``` 범위 내), 만약 이미지의 크기가 커질때를 고려하여 큰 타입을 잡았다. 

#### 1.3.7 ```pow()```in ```math.h``` v.s. simple square
...
이 코드로 테스트를 해본 결과, 각 방법의 시행시간은 무려 11배나 차이 났다. 
